...
function ParDeBarreiras(altura, abertura, x) {
  ...
  this.elemento.appendChild(this.superior.elemento); // superior é uma Barreira e contem um elemento
...
function Barreiras(altura, largura, abertura, espaco, notificarPontoF) {
  this.pares = [
  ...
  this.animar = () => {
    this.pares.forEach((par) => {
      ...
      //quando o elemento sar da área do jogo
      ...
      if (cruzouOMeio) notificarPontoF();
    });
  };
}
...
function Passaro(alturaDoJogo) {
  ...
  this.animar = () => {
    ...
    if (novoY <= 0) {
      ...
      this.setY(this.setY(alturaMaxima));
    } else {
      this.setY(novoY);
    }
  };
...
function FlappyBird() {
  ...
  const areaDoJogo = document.querySelector("[fie-jogo]");
